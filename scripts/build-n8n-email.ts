...
const masterTemplate = readFileSync(
  join(root, 'email-templates/master/master-template.html'),
  'utf8',
);
const contentTemplate = readFileSync(
  join(root, 'email-templates/templates/lead-intake.html'),
  'utf8',
);
...
// Write workflow JSON
writeFileSync(
  join(root, 'workflows/n8n/branded-lead-intake.json'),
...
const emailNode = workflow.nodes.find(
  (n) => n.name === 'Send Email Notification',
);
if (!emailNode) {
...
if (params.emailFormat !== 'html') {
  console.error(
    `❌ VALIDATION FAILED: emailFormat is "${params.emailFormat}", expected "html"`,
  );
  process.exit(1);
}
...
if (typeof params.html !== 'string' || params.html.length < 1000) {
  console.error(
    `❌ VALIDATION FAILED: html content missing or too small (${String(params.html).length} bytes)`,
  );
  process.exit(1);
}
...
if (process.argv.includes('--deploy')) {
  ...
  if (!N8N_API_KEY || !WORKFLOW_ID) {
    console.error(
      '❌ Missing required environment variables for --deploy: N8N_API_KEY, N8N_WORKFLOW_ID',
    );
    process.exit(1);
  }
  ...
  const deployed = (await verifyResponse.json()) as {
    nodes: Array<{name: string; parameters: Record<string, unknown>}>;
  };
  const deployedEmailNode = deployed.nodes.find(
    (n) => n.name === 'Send Email Notification',
  );
  ...
  const deployedHtmlLength = String(
    deployedEmailNode?.parameters?.html || '',
  ).length;
  if (deployedHtmlLength < 1000) {
    console.error(
      `❌ VERIFY FAILED: Deployed html content too small (${deployedHtmlLength} bytes)`,
    );
    process.exit(1);
  }

  console.log(
    `✅ Verified: emailFormat=html, html=${deployedHtmlLength} bytes`,
  );
