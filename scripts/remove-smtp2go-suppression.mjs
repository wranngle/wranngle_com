...
async function removeSupression() {
  ...
  try {
    ...
    await page.goto('https://app.smtp2go.com/login/', {
    ...
    console.log('Form structure:', formHTML.slice(0, 1000));
    ...
    for (const sel of emailSelectors) {
      ...
      if (element) {
        ...
        await element.fill(SMTP2GO_USER);
        break;
      }
    }
    ...
    for (const sel of passSelectors) {
      ...
      if (element) {
        ...
        await element.fill(SMTP2GO_PASS);
        break;
      }
    }
    ...
    await page.screenshot({
      path: String.raw`C:\Users\root\Documents\dev\wranngle.com\smtp2go-before-login.png`,
    });
    ...
    await page.screenshot({
      path: String.raw`C:\Users\root\Documents\dev\wranngle.com\smtp2go-after-login.png`,
    });
    ...
    await page.goto(`${baseUrl}/reports/suppressions/`, {
      waitUntil: 'domcontentloaded',
    });
    await page.waitForTimeout(5000);
    ...
    // Screenshot
    await page.screenshot({
      path: String.raw`C:\Users\root\Documents\dev\wranngle.com\smtp2go-suppressions.png`,
    });
    ...
    if (pageContent.includes(EMAIL_TO_REMOVE)) {
      ...
      const checkbox = await page
        .locator(`tr:has-text("${EMAIL_TO_REMOVE}") input[type="checkbox"]`)
        .first();
      if ((await checkbox.count()) > 0) {
        ...
        const bulkDelete = await page
          .locator(
            'button:has-text("Delete Selected"), a:has-text("Delete Selected"), .delete-selected',
          )
          .first();
    ...
    // Final screenshot
    await page.screenshot({
      path: String.raw`C:\Users\root\Documents\dev\wranngle.com\smtp2go-final.png`,
    });
    console.log('Done! Check screenshots for results.');
  } catch (error) {
    console.error('Error:', error.message);
    await page.screenshot({
      path: String.raw`C:\Users\root\Documents\dev\wranngle.com\smtp2go-error.png`,
    });
  } finally {
    await page.waitForTimeout(3000);
    await browser.close();
