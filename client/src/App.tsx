...
const IntakeForm = ({ selectedPackage, onSuccess }) => {
    ...
    return (
      <div className="bg-[#f0f0f0] p-6 rounded-sm shadow-xl max-w-sm mx-auto font-mono text-black relative border-t-8 border-[var(--s500)]">
        ...
        <div className="space-y-2 text-sm mb-6">
          <div className="flex justify-between"><span>REF:</span><span>{Math.random().toString(36).substr(2, 9).toUpperCase()}</span></div>
          <div className="flex justify-between"><span>DATE:</span><span>{new Date().toLocaleDateString()}</span></div>
          <div className="border-b border-dashed border-gray-400 my-2" />
          <div className="flex justify-between font-bold"><span>ITEM</span><span>AMT</span></div>
          <div className="flex justify-between">
            <span>{successData.package === 'premium' ? 'ELITE AGENT' : 'STARTER AGENT'}</span>
            <span>${successData.package === 'premium' ? '500' : '250'}</span>
          </div>
          <div className="border-b border-dashed border-gray-400 my-2" />
          <div className="flex justify-between font-bold text-lg"><span>TOTAL</span><span>${successData.package === 'premium' ? '500.00' : '250.00'}</span></div>
        </div>
        ...
        <DialogClose asChild>
          <Button variant="outline" className="w-full mt-6 border-black/20 text-black hover:bg-black hover:text-white transition-colors">
            CLOSE RECEIPT
          </Button>
        </DialogClose>
      </div>
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle className="brand-font text-2xl">Agent Intake Form</DialogTitle>
        <DialogDescription>Tell us about your business to get your agent ready.</DialogDescription>
      </DialogHeader>
      ...
      {currentPackage === 'basic' && (
        <div className="mt-4 p-4 border border-[var(--s500)]/30 bg-[var(--s500)]/5 rounded-lg flex gap-4 items-start">
          ...
          <div>
            <div className="text-xs font-bold text-[var(--s500)] uppercase tracking-wider mb-1">Recommended Upgrade</div>
            <p className="text-[11px] opacity-80 leading-relaxed mb-2">
              Best Practice: 84% of trade businesses see 2x lead conversion when combining <b>Voice Agent</b> with <b>Web Chat</b>.
            </p>
            <button 
              type="button"
      ...
      {currentPackage === 'premium' && (
        <div className="mt-4 p-4 border border-[var(--s500)]/30 bg-[var(--s500)]/10 rounded-lg flex gap-4 items-center">
          ...
          <div>
            <div className="text-xs font-bold text-[var(--s500)] uppercase tracking-wider">Elite Agent Secured</div>
      ...
      <form onSubmit={handleSubmit((data) => mutation.mutate(data))} className="space-y-4 py-4">
        <div className="grid gap-2">
          ...
          <Input id="businessName" className="placeholder:text-white/20 border-l-4 border-l-[var(--s500)]" {...register("businessName", { required: true })} placeholder="Apex Plumbing" />
        </div>
        <div className="grid gap-2">
          <Label htmlFor="industry">Industry / Trade</Label>
          <Input id="industry" className="placeholder:text-white/20 border-l-4 border-l-[var(--s500)]" {...register("industry", { required: true })} placeholder="HVAC / Electrical / etc." />
        </div>
        <div className="grid gap-2">
          <Label htmlFor="ownerName">Contact Person</Label>
          <Input id="ownerName" className="placeholder:text-white/20 border-l-4 border-l-[var(--s500)]" {...register("ownerName", { required: true })} placeholder="John Doe" />
        </div>
        <div className="grid gap-2">
          <Label htmlFor="email">Email Address</Label>
          <Input id="email" type="email" className="placeholder:text-white/20 border-l-4 border-l-[var(--s500)]" {...register("email", { required: true })} placeholder="john@example.com" />
        </div>
        <div className="grid gap-2">
        ...
        <Button type="submit" className="w-full bg-[var(--s500)] hover:bg-[var(--s500)]/90" disabled={mutation.isPending}>
          {mutation.isPending ? "Initializing..." : `Confirm ${currentPackage === 'premium' ? 'Elite' : 'Starter'} Deployment`}
        </Button>
      </form>
    </>
  );
...
const PricingCard = ({ pkg, isDark }) => {
  const factsData = pkg.id === 'premium' ? {
    tierName: "Elite",
    discountPercent: 20,
    pricing: { monthly: 500, annualMonthly: 400.00, addon: 250 },
    specs: { 
      coverage: "24/7/365", 
      ingredients: "PREMIUM NEURAL AUDIO (ELEVENLABS), DUAL-AGENT ON-CALL LOGIC, CAL.COM SYNC, 10DLC COMPLIANT SMS, UNIFIED INBOX ENGINE."
    },
    ...
    tierName: "Core",
    discountPercent: 15,
    pricing: { monthly: 250, annualMonthly: 212.50, addon: 250 },
    specs: { 
      coverage: "After Hours", 
      ingredients: "NATURAL LANGUAGE PROCESSING, LEAD SCORING ALGORITHM, TWILIO VOICE STACK, SHARED SMS POOL, ONE-WAY NOTIFICATIONS, 100% AUTOMATION."
    },
  ...
  return (
    <div className="relative group h-full">
      {/* Main Card */}
      <div className={`relative h-full p-8 rounded-[24px_4px_24px_4px] border-y border-r border-l-4 border-l-[var(--s500)] ${isDark ? 'border-white/10 bg-[#18181b]' : 'border-black/5 bg-white'} flex flex-col noise-overlay overflow-hidden`}>
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <div className="flex-1 flex flex-col">
            <div className="text-4xl font-bold mb-6">${pkg.price}<span className="text-sm font-normal opacity-50">/mo</span></div>
          ...
          <Dialog>
            <DialogTrigger asChild>
              <Button className={`w-full ${pkg.id === 'premium' ? 'bg-[var(--v500)] hover:bg-[var(--v500)]/90 hover:scale-[1.02] transition-all' : isDark ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-black/10 text-black hover:bg-black/20'}`}>
                Select {pkg.name}
              </Button>
            </DialogTrigger>
            <DialogContent className={isDark ? "bg-[#12111a] text-white border-white/10" : "bg-white text-black border-black/10"}>
  ...
  return (
    <section id="pricing" className="py-24 px-6 max-w-7xl mx-auto w-full">
      <div className="text-center mb-16">
        <h2 className="brand-font text-4xl font-bold mb-4">Scale Your Operations Instantly.</h2>
      </div>
      <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
        {[
          { id: 'basic', name: 'Core Agent', price: '250', features: ['Voice-only AI Agent', 'After-hours coverage', 'SMS Follow-up', 'Email lead capture', 'Basic Support'] },
          { id: 'premium', name: 'Elite Agent', price: '500', features: ['Triple-channel Voice + Web + SMS AI Agents', '24/7 Priority coverage', 'Calendar Integration', 'Direct Transfer capability', 'Custom Voice Identity', 'Priority Support'] },
        ].map((pkg) => (
    ...
    const handleOutsideClick = (e) => {
      ...
      if (widget && !widget.contains(e.target)) {
        ...
        if (rect.height > 80 && widget.shadowRoot) {
          ...
          if (!isDialogOpen) {
             widget.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', code: 'Escape', keyCode: 27, which: 27, bubbles: true }));
          }
          ...
          const closeBtn = buttons.find(b => {
            ...
            return label.includes('close') || label.includes('minimize') || label.includes('collapse');
          });
      ...
      <div className={`min-h-screen flex flex-col ${isDark ? 'bg-page-dark text-[#fcfaf5]' : 'bg-page-light text-[#12111a]'}`}>
            ...
            >
              <nav className="flex flex-col gap-6 text-2xl font-bold brand-font">
                ...
                <div className="flex items-center justify-between py-6 border-y border-black/10 dark:border-white/10">
                  <span className="text-sm font-medium opacity-60 uppercase tracking-widest">Toggle Appearance</span>
                  <ThemeToggle isDark={isDark} toggle={toggleTheme} />
                </div>
                <Dialog>
                  <DialogTrigger asChild>
        ...
        <main className="flex-1 max-w-7xl mx-auto px-6 py-20 grid lg:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <p className="text-lg opacity-80 mb-10 max-w-md">The 24/7 AI Voice Agent for small trades businesses. Stop missing leads after hours.</p>
        ...
        <section id="features" className="py-32 px-6 max-w-7xl mx-auto w-full relative">
          ...
          <div className="mb-24 relative z-10">
            <h2 className="brand-font text-5xl md:text-6xl font-bold mb-6 max-w-3xl leading-tight">
              Industrial-Grade <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[var(--s500)] to-[var(--v500)]">Defense System</span> <br />
              For Your Revenue.
            </h2>
            <p className="opacity-60 max-w-xl text-lg leading-relaxed">
              Manual reception is a vulnerability. Wranngle deploys an autonomous, deterministic layer that insulates your business from chaos.
            </p>
          </div>
          ...
          <div className="grid lg:grid-cols-3 gap-8 relative z-10">
            ...
            <TerminalCard isDark={isDark} title="TEMPORAL_SENTINEL" status="SCANNING" index="01">
              <RadarWatchdog />
              <div className="mt-8 relative z-10">
                <h3 className="brand-font text-2xl font-bold mb-2">Total Temporal Coverage</h3>
                <p className="text-sm opacity-60 leading-relaxed">
                  A persistent, non-sleeping observer. Whether it's 2 AM or Christmas Day, the sentinel intercepts every signal.
                </p>
              </div>
            </TerminalCard>

            {/* Feature 2: The Spectral Gate */}
            <TerminalCard isDark={isDark} title="SPECTRAL_GATE" status="FILTERING" index="02">
               <SpectralAnalyzer />
               <div className="mt-8 relative z-10">
                <h3 className="brand-font text-2xl font-bold mb-2">Signal Validation</h3>
                <p className="text-sm opacity-60 leading-relaxed">
                  Advanced heuristics analyze caller intent in real-time. Spam is vaporized. Revenue is crystalized.
                </p>
              </div>
            </TerminalCard>

            {/* Feature 3: The Synapse Bridge */}
            <TerminalCard isDark={isDark} title="SYNAPSE_UPLINK" status="CONNECTED" index="03">
              <SynapseLink />
              <div className="mt-8 relative z-10">
                <h3 className="brand-font text-2xl font-bold mb-2">Zero-Latency Uplink</h3>
                <p className="text-sm opacity-60 leading-relaxed">
                  Valid leads are transmuted into structured data and injected directly into your mobile device via SMS.
                </p>
              </div>
            </TerminalCard>
          </div>
...
const ButtonPrimary = React.forwardRef(({ children, ...props }, ref) => (
  <button ref={ref} {...props} className="px-8 py-4 bg-[var(--s500)] text-white font-bold uppercase text-xs rounded-lg shadow-lg hover:scale-105 transition-all flex items-center gap-2">
    {children} <ArrowRight size={14} />
  </button>
));

const ButtonGhost = ({ children, ...props }) => (
  <button {...props} className="px-8 py-4 border border-current font-bold uppercase text-xs rounded-lg hover:bg-white/5 transition-all">{children}</button>
);

const Card = ({ title, desc, accent, isDark }) => (
  <div className="relative group">
    <div className={`absolute inset-0 rounded-[var(--lasso)] translate-x-1.5 translate-y-1.5 transition-all duration-300 group-hover:translate-x-3 group-hover:translate-y-3 ${accent ? 'bg-[var(--s500)]' : 'bg-[var(--v500)]'}`} />
    <div className={`relative h-full p-8 rounded-[var(--lasso)] border border-white/10 ${isDark ? 'bg-[#18181b]' : 'bg-white'} shadow-sm`}>
      <h3 className="brand-font text-xl font-bold mb-2">{title}</h3>
      <p className={`text-sm opacity-70 leading-relaxed ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{desc}</p>
    </div>
  </div>
);
...
const ConsoleVisual = ({ isDark, lines }) => {
  ...
  return (
    <div ref={containerRef} className={`relative h-80 w-full rounded-[24px_4px_24px_4px] border-y border-r border-l-4 border-l-[var(--s500)] ${isDark ? 'border-white/10 bg-[#0f0f13] text-gray-400' : 'border-black/10 bg-[#1a1a1e] text-gray-400'} p-6 mono-font text-[11px] flex flex-col overflow-hidden noise-overlay`} style={{ boxShadow: 'var(--shadow-card)' }}>
  ...
  return (
    <div className="h-48 w-full flex items-center justify-center relative overflow-hidden bg-black/20 rounded-lg border border-white/5">
      ...
      {[60, 120, 180].map((size, i) => (
        <div key={i} className="absolute rounded-full border border-[var(--s500)] opacity-20" style={{ width: size, height: size }} />
      ))}
  ...
  return (
    <div className="h-48 w-full flex flex-col justify-between p-4 bg-black/20 rounded-lg border border-white/5 relative overflow-hidden">
          ...
          {Array.from({ length: 20 }).map((_, i) => (
            <motion.div
              ...
              animate={{ 
                ...
                backgroundColor: i > 12 ? ["#cf3c69", "#cf3c69"] : ["#ff5f00", "#ff5f00"] // Red for high freq (spam)
              }}
              transition={{ 
                duration: 0.5, 
                repeat: Infinity, 
       ...
       <div className="mt-2 h-6 w-full bg-black/40 rounded flex items-center px-2 font-mono text-[9px] text-green-400 gap-2">
         ...
         <TypewriterSequence sequence={["> VOICE_MATCH_CONFIRMED", "> SENTIMENT: POSITIVE", "> ROUTING_TO_SALES"]} />
       </div>
    </div>
  );
};
  ...
  return (
    <div className="h-48 w-full flex items-center justify-center relative bg-black/20 rounded-lg border border-white/5">
       ...
       <svg className="absolute inset-0 w-full h-full pointer-events-none">
          <path d="M 60 96 C 150 96, 250 96, 300 96" stroke="rgba(255,255,255,0.1)" strokeWidth="2" fill="none" />
          <path d="M 60 96 C 150 96, 250 96, 300 96" stroke="url(#gradient)" strokeWidth="2" fill="none" strokeDasharray="5 5" />
          <defs>
...
const TerminalCard = ({ children, title, status, index, isDark }) => (
  <div className="relative group">
    {/* Main Card */}
    <div className={`relative h-full p-1 rounded-[24px_4px_24px_4px] border-y border-r border-l-4 border-l-[var(--s500)] ${isDark ? 'border-white/10 bg-[#12111a]' : 'border-black/10 bg-white'} overflow-hidden noise-overlay`} style={{ boxShadow: 'var(--shadow-card)' }}>
      {/* Holographic Border Effect */}
      <div className={`absolute inset-0 border border-white/5 rounded-[24px_4px_24px_4px] bg-gradient-to-b from-white/5 to-transparent backdrop-blur-sm transition-colors group-hover:border-[var(--s500)]/50 pointer-events-none z-10`} />
      ...
      <div className="relative p-6 h-full flex flex-col z-20">
        ...
        <div className="flex justify-between items-center mb-6 font-mono text-[10px] tracking-widest text-[var(--s500)]">
          <span className="border border-[var(--s500)]/30 px-2 py-0.5 rounded bg-[var(--s500)]/5">MOD_{index} // {title}</span>
...
const TypewriterSequence = ({ sequence }) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentLine((prev) => (prev + 1) % sequence.length);
    }, 2000);
    return () => clearInterval(interval);
  }, [sequence]);
...
const AgentFactsLabel = ({ tierName, pricing, specs, limits, features, discountPercent }) => {
  return (
    <div className="bg-white border-2 border-black p-4 w-full max-w-[380px] font-sans text-black shadow-sm mx-auto flex flex-col h-full">
      ...
      <div className="flex-grow">
        ...
        <div className="border-b-[8px] border-black pb-1 mb-1">
          <h1 className="text-4xl font-black leading-none tracking-tighter uppercase italic">AI Agent Facts</h1>
        ...
        <div className="border-b-[4px] border-black py-1">
          <div className="text-xs font-bold uppercase tracking-tighter">Base Service Price</div>
          <div className="flex justify-between items-center">
            <div>
              <span className="text-2xl font-black block leading-none">Monthly</span>
              <span className="text-[10px] font-bold uppercase">No Commitment</span>
            </div>
            <span className="text-5xl font-black leading-none">${pricing.monthly}</span>
          </div>
          
          {/* Annual Discount Section */}
          <div className="mt-2 pt-1 border-t border-black border-dashed flex justify-between items-center">
            <div>
              <div className="text-[11px] font-black uppercase">{discountPercent}% Discount Price</div>
              <div className="text-[9px] leading-tight text-gray-600 font-bold uppercase">w/ Annual Commitment</div>
            </div>
            <div className="text-right">
              <span className="text-2xl font-black">${pricing.annualMonthly}</span>
        ...
        <div className="border-b-[4px] border-black">
          ...
          <div className="flex justify-between items-baseline border-b border-black py-1 text-sm">
            <span><span className="font-bold">Voice Coverage</span> {specs.coverage}</span>
            <span className="font-bold text-xs italic">ACTIVE</span>
          </div>

          {/* Minutes Stat */}
          <div className="flex justify-between items-baseline border-b border-black py-1 text-sm">
            <span><span className="font-bold">Total Voice Minutes</span></span>
          ...
          {/* SMS Stat */}
          <div className="flex justify-between items-baseline border-b border-black py-1 text-sm">
            <span><span className="font-bold">Total SMS Segments</span></span>
            <span className="font-bold">100%</span>
          </div>
          <div className="pl-4 text-xs py-1 border-b border-black">
          ...
          <div className="py-2">
            <div className="font-bold text-sm mb-1 uppercase tracking-tighter">Included System Features:</div>
            <ul className="text-xs space-y-1">
              {features.map((feature, index) => (
                <li key={index} className="flex justify-between border-b border-gray-200 pb-0.5 last:border-0">
      ...
      <div className="mt-auto">
        ...
        <div className="text-[9px] mt-2 leading-tight">
          <span className="font-bold uppercase italic">Marketing Ingredients:</span> {specs.ingredients}
        </div>

        {/* Footnote */}
        <div className="text-[8px] mt-2 border-t border-black pt-1 leading-[1.2]">
          * The % Fair Use (FU) indicates the capacity included in the base price before standard overage rates apply. 
          Mid-annual cancellation of the Discount Price is subject to cancellation fees. 
          API access not available. White-labeling included. All agents are trade-specific.
        </div>
      </div>
    </div>
  );
