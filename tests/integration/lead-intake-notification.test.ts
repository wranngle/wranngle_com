...
function loadEnvKey(key: string): string {
	...
	const envPath = join(process.env.USERPROFILE || process.env.HOME || '', '.claude', '.env');
	if (existsSync(envPath)) {
		const match = readFileSync(envPath, 'utf-8').split('\n').find(l => l.startsWith(`${key}=`));
...
async function getLatestExecution(workflowId: string) {
	const response = await fetch(`${N8N_API}/executions?workflowId=${workflowId}&limit=1`, {
		headers: {'X-N8N-API-KEY': N8N_API_KEY},
	});
	const data = await response.json() as {data: Array<{id: string; status: string; startedAt: string; stoppedAt: string; finished: boolean}>};
	return data.data?.[0];
}
...
describe('Lead Intake Notification Flow', () => {
	...
	describe('MOCKED: Variable mapping validation', () => {
		it('should call UMS with lead-intake template via direct test', async () => {
			...
			const data = await response.json() as {success?: boolean; request_id?: string; error?: string};
